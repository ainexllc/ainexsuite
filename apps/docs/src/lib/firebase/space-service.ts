...
import type { DocSpace, DocSpaceDoc, DocSpaceDraft, SpaceMember, SpaceType } from "@/lib/types/doc";

// Use unified "spaces" collection to match the provider
const SPACES_COLLECTION = "spaces";

function convertToDocSpace(id: string, data: DocSpaceDoc): DocSpace {
  return {
    id,
    name: data.name,
    type: data.type,
...
export async function getSpace(spaceId: string): Promise<DocSpace | null> {
  const spaceRef = doc(db, SPACES_COLLECTION, spaceId);
  const snapshot = await getDoc(spaceRef);
  if (!snapshot.exists()) {
    return null;
  }
  return convertToDocSpace(snapshot.id, snapshot.data() as DocSpaceDoc);
}

export function subscribeToSpaces(
  userId: string,
  onData: (spaces: DocSpace[]) => void,
  ...
  return onSnapshot(
    ...
    (snapshot) => {
      const spaces = snapshot.docs.map((docSnap) =>
        convertToDocSpace(docSnap.id, docSnap.data() as DocSpaceDoc)
      );
      onData(spaces);
    },
    (error) => {
