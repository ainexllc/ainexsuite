...
import { clientDocCollection } from "@/lib/firebase/client-collections";
...
export async function deleteLabel(userId: string, labelId: string) {
  ...
  // Remove label reference from docs.
  const docsWithLabelQuery = query(
    clientDocCollection(userId),
    where("labelIds", "array-contains", labelId),
  );

  const snapshot = await getDocs(docsWithLabelQuery);
