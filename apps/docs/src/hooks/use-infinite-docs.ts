...
import { getUserDocs } from '@/lib/firebase/doc-service';
...
export function useInfiniteDocs({
  userId,
  spaceId,
  enabled = true,
}: UseInfiniteNotesOptions): UseInfiniteNotesResult {
  const [docs, setNotes] = useState<Doc[]>([]);
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      ...
      const { docs: fetchedNotes, lastDoc } = await getUserDocs(userId, {
        limit: BATCH_SIZE,
        sortBy: 'updatedAt',
        sortOrder: 'desc',
        spaceId,
      ...
      setError('Failed to load docs. Please try refreshing the page.');
    } finally {
      setIsLoading(false);
    }
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load more docs:', err);
