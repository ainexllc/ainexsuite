...
/**
 * Calculate writing streak and docs this week from doc timestamps
 */
function calculateStreak(
  docs: Array<{ createdAt: Date; updatedAt: Date }>
): StreakData {
  if (docs.length === 0) return { streakDays: 0, docsThisWeek: 0 };

  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const weekAgo = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000);

  // Count docs this week
  const docsThisWeek = docs.filter((n) => n.createdAt >= weekAgo).length;

  // Calculate streak: count consecutive days with docs going backwards from today
  const docDates = new Set(
    docs.map((n) => {
      const d = n.createdAt;
  ...
  return { streakDays, docsThisWeek };
}
...
function buildNoteSections(
  ...
  if (streakData && (streakData.streakDays > 0 || streakData.docsThisWeek > 0)) {
    allSections.push({
      animatedIconType: "Sparkle",
      label: "Writing Streak",
      content:
        streakData.streakDays > 0
          ? `${streakData.streakDays}-day streak! You've created ${streakData.docsThisWeek} docs this week`
          : `${streakData.docsThisWeek} docs this week - start your streak today!`,
      gradient: { from: primaryColor, to: "#f97316" },
    });
  }
...
export function useWorkspaceInsights(): WorkspaceInsightsResult {
  ...
  // Filter to recent active docs
  const recentNotes = useMemo(() => {
    const active = docs.filter((n) => !n.archived && !n.deletedAt);
  ...
  // All active docs for streak calculation
  const activeNotes = useMemo(() => {
    return docs.filter((n) => !n.archived && !n.deletedAt);
  }, [docs]);

  // Config for the shared hook
  const config: WorkspaceInsightsConfig<Doc, DocsInsightData> = useMemo(
    () => ({
      appName: "docs",
