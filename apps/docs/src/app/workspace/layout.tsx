...
import type { SpaceMember as DocSpaceMember } from '@/lib/types/doc';
...
export default function WorkspaceRootLayout({
  ...
  const quickActions = getQuickActionsForApp('docs');
  ...
  const handleQuickAction = useCallback((actionId: string) => {
    switch (actionId) {
      case 'new-doc':
        ...
        router.push('/workspace?action=new-doc');
        break;
      case 'new-checklist':
  ...
  return (
    <HintsProvider>
      ...
      {editingSpace && (
        <SpaceSettings
          ...
          space={{
            ...
            members: (editingSpace.members || []).map((m: DocSpaceMember) => ({
              uid: m.uid,
          ...
          canEdit={
            (editingSpace.members || []).find((m: DocSpaceMember) => m.uid === user.uid)?.role === 'admin'
          }
          canDelete={
      ...
      <AppFloatingDock currentApp="docs" />
    </HintsProvider>
  );
}
