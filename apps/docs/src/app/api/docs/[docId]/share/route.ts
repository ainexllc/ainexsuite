...
import { adminDocDoc } from "@/lib/firebase/admin-collections";
...
export async function POST(request: NextRequest) {
  ...
  const docRef = adminDocDoc(user.uid, docId);
  ...
  if (collaboratorRecord.uid === user.uid) {
    return NextResponse.json({ error: "You already own this doc" }, { status: 400 });
  }
