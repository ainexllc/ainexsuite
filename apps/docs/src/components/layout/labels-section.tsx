...
import type { Label } from "@/lib/types/doc";
...
export function LabelsSection({ onClose }: LabelsSectionProps) {
  ...
  const { activeLabelIds, setActiveLabelIds, docs } = useDocs();
  ...
  // Count docs per label
  const docCounts = useMemo(() => {
    const counts = new Map<string, number>();
    docs.forEach(doc => {
      doc.labelIds?.forEach((labelId) => {
  ...
  return (
    <NavigationSection title="Labels">
      <div className="space-y-1">
        {labelsLoading ? (
          ...
          labelNodes.map(({ label, depth }) => {
            ...
            const docCount = docCounts.get(label.id) || 0;
      ...
      <ConfirmationDialog
        ...
        description={`This will remove the tag from ${docCounts.get(deletingLabel?.id || "") || 0} doc${(docCounts.get(deletingLabel?.id || "") || 0) === 1 ? "" : "s"}. This action cannot be undone.`}
