...
export function ReminderList() {
  ...
  const entries = useMemo(() => {
    return reminders.map((reminder) => {
      const doc = docs.find((item) => item.id === reminder.docId) ?? null;
  ...
  const handleSnooze = async (reminderId: string, minutes: number) => {
    ...
    try {
      ...
      await updateDoc(entry.reminder.docId, {
        reminderAt: newDate,
      });
    } finally {
    ...
    <ListSection title={label} count={items.length}>
      {items.length ? (
        <ul className="space-y-3">
          {items.map(({ reminder, doc, timestamp, title }) => {
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted">
                      {doc?.labelIds?.length ? `${doc.labelIds.length} labels · ` : ""}
