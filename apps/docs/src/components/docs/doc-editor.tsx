...
import type { BackgroundOverlay } from "@/lib/types/doc";
...
export function DocEditor({ doc, onClose }: DocEditorProps) {
  const {
    updateDoc,
    duplicateDoc,
    togglePin,
  ...
  const [mode, setMode] = useState<"text" | "checklist">(
    doc.type === "checklist" ? "checklist" : "text",
  );
  const [checklist, setChecklist] = useState<ChecklistItem[]>(
    doc.checklist,
  );
  const [color, setColor] = useState<DocColor>(doc.color);
  const [backgroundImage, setBackgroundImage] = useState<string | null>(doc.backgroundImage ?? null);
  const [backgroundOverlay, setBackgroundOverlay] = useState<BackgroundOverlay>(doc.backgroundOverlay ?? 'auto');
  ...
  const [priority, setPriority] = useState<DocPriority>(doc.priority ?? null);
  const [showPriorityPicker, setShowPriorityPicker] = useState(false);
  const [existingAttachments, setExistingAttachments] = useState<DocAttachment[]>(
    doc.attachments,
  );
  const [removedAttachments, setRemovedAttachments] = useState<DocAttachment[]>([]);
  ...
  const lastSavedBodyRef = useRef(doc.body);
  ...
  const [reminderEnabled, setReminderEnabled] = useState(Boolean(doc.reminderAt));
  const [reminderValue, setReminderValue] = useState(
    doc.reminderAt ? formatDateTimeLocalInput(doc.reminderAt) : "",
  );
  ...
  const currentColorConfig = useMemo(() => {
    return DOC_COLORS.find((c) => c.id === color) || DOC_COLORS[0];
  }, [color]);
  ...
  const [selectedSpaceId, setSelectedSpaceId] = useState<string>(doc.spaceId || "personal");
  ...
  const handleAutoSave = useCallback(async (data: typeof autoSaveData) => {
    const updates: DocDraft = {};

    if (data.title !== doc.title) updates.title = data.title;
    if (data.body !== doc.body) updates.body = data.body;
    if (data.type !== doc.type) {
      updates.checklist = data.checklist;
      updates.body = data.body;
    } else if (data.type === "checklist") {
      const checklistChanged = JSON.stringify(data.checklist) !== JSON.stringify(doc.checklist);
      if (checklistChanged) updates.checklist = data.checklist;
    }
    if (data.color !== doc.color) updates.color = data.color;
    if (JSON.stringify(data.labelIds) !== JSON.stringify(doc.labelIds ?? [])) {
      updates.labelIds = data.labelIds;
    }
    const docSpaceId = doc.spaceId || undefined;
    if (data.spaceId !== docSpaceId) updates.spaceId = data.spaceId;
  ...
  const handleBackgroundChange = useCallback(async (newBackgroundImage: string | null) => {
    ...
    await updateDoc(doc.id, { backgroundImage: newBackgroundImage });
  }, [doc.id, updateDoc]);

  const handleOverlayChange = useCallback(async (newOverlay: BackgroundOverlay) => {
    setBackgroundOverlay(newOverlay);
    // Auto-save to database
    await updateDoc(doc.id, { backgroundOverlay: newOverlay });
  }, [doc.id, updateDoc]);
  ...
  const existingReminder = useMemo(() => {
    ...
    return reminders.find((item) => item.id === doc.reminderId) ?? null;
  }, [reminders, doc.reminderId]);
  ...
  const handleFinalizeAndClose = useCallback(async () => {
    ...
    // Handle pin changes
    const pinnedChanged = pinned !== doc.pinned;

    if (pinnedChanged) {
      await togglePin(doc.id, pinned);
    }

    // Handle priority changes
    const priorityChanged = priority !== (doc.priority ?? null);
    if (priorityChanged) {
      await updateDoc(doc.id, { priority });
    }
    ...
    if (removedAttachments.length > 0) {
      await Promise.all(
        removedAttachments.map((attachment) =>
          removeAttachment(doc.id, attachment),
        ),
      );
    }
    ...
    const shouldRemoveReminder = !reminderEnabled && Boolean(doc.reminderId);
    const shouldCreateReminder = reminderEnabled && !doc.reminderId && reminderFireAt;
    const shouldUpdateExisting = reminderEnabled && Boolean(doc.reminderId) && Boolean(reminderFireAt);
    const reminderTimeChanged =
      reminderEnabled && reminderFireAt &&
      (!doc.reminderAt || doc.reminderAt.getTime() !== reminderFireAt.getTime());
    ...
    const reminderFrequencyChanged =
      reminderEnabled && existingReminder
        ...
        : reminderEnabled && !doc.reminderId && reminderFrequency !== "once";
    ...
    if (shouldRemoveReminder && doc.reminderId) {
      await deleteReminder(doc.reminderId);
      await updateDoc(doc.id, { reminderAt: null, reminderId: null });
    }
    ...
    if (shouldCreateReminder && reminderFireAt) {
      ...
      await updateReminder(doc.reminderId, {
        fireAt: reminderFireAt,
        channels: activeChannels,
        frequency: reminderFrequency,
        customCron: reminderFrequency === "custom" ? customCron || null : null,
  ...
  useEffect(() => {
    if (!doc.reminderId && !reminderEnabled) {
      setReminderChannels([...preferences.reminderChannels]);
    }
  }, [preferences.reminderChannels, doc.reminderId, reminderEnabled]);
  ...
  const handleRemoveExistingAttachment = (attachment: DocAttachment) => {
            ...
            <div className="flex items-center gap-2">
              ...
              <div className={clsx(
                ...
                <button
                  type="button"
                  onClick={async () => {
                    const newNoteId = await duplicateDoc(doc.id);
                    if (newNoteId) {
                      onClose();
                    }
                  }}
