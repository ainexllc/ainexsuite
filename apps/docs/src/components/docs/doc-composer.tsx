...
import type { ChecklistItem, DocColor, DocPriority } from "@/lib/types/doc";
...
import { InlineCalculator } from "@/components/docs/inline-calculator";
import { InlineCalendar } from "@/components/docs/inline-calendar";
...
const DOC_TEMPLATES: NoteTemplate[] = [
  {
    id: "blank",
    label: "Blank",
    icon: "📝",
...
export function DocComposer({ onManagePeople, onManageSpaces }: DocComposerProps) {
  ...
  const [color, setColor] = useState<DocColor>("default");
  const [pinned, setPinned] = useState(false);
  const [priority, setPriority] = useState<DocPriority>(null);
  const [showPriorityPicker, setShowPriorityPicker] = useState(false);
  const [attachments, setAttachments] = useState<AttachmentDraft[]>([]);
  const [isSubmitting, setIsSubmitting] = useState(false);
  // Auto-create: track created doc ID for auto-save
  ...
  // Auto-create doc on first content
  useEffect(() => {
    ...
    const createDraftNote = async () => {
      ...
      try {
        ...
        if (docId) {
          setCreatedNoteId(docId);
        }
      } catch (error) {
        console.error("[DocComposer] Failed to auto-create doc:", error);
      } finally {
        isCreatingRef.current = false;
      }
    };
  ...
  }, [expanded, createdNoteId, title, body, mode, checklist, color, createDoc]);

  // Auto-save changes to created doc (debounced)
  ...
  useEffect(() => {
    ...
    autoSaveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        console.error("[DocComposer] Auto-save failed:", error);
      }
    }, 1500);
  ...
  }, [createdNoteId, title, body, mode, checklist, color, selectedLabelIds, docDate, updateDoc]);
  ...
  type EnhanceStyle = "professional" | "casual" | "concise" | "grammar" | "bullet" | "prose" | "shorter" | "longer" | "headlines" | "eli5";

  const ENHANCE_STYLES: { id: EnhanceStyle; label: string; description: string; icon: LucideIcon }[] = [
    { id: "professional", label: "Professional", description: "Polished & formal tone", icon: Briefcase },
    { id: "casual", label: "Casual", description: "Friendly & conversational", icon: MessageCircle },
    { id: "concise", label: "Concise", description: "Brief & to the point", icon: Scissors },
    { id: "grammar", label: "Fix Grammar", description: "Fix spelling & grammar", icon: CheckCircle },
    { id: "bullet", label: "Bullet Points", description: "Convert to bullet list", icon: List },
    { id: "prose", label: "To Prose", description: "Convert to paragraphs", icon: AlignLeft },
    { id: "shorter", label: "Shorten", description: "Reduce length by ~50%", icon: ArrowDownWideNarrow },
    { id: "longer", label: "Expand", description: "Add more detail", icon: ArrowUpWideNarrow },
    { id: "headlines", label: "Headlines", description: "Extract key points", icon: Newspaper },
    { id: "eli5", label: "Simplify", description: "Explain like I'm 5", icon: Baby },
  ];
  ...
  const handleEnhanceBody = async (style: EnhanceStyle) => {
    ...
    try {
      ...
      const taskMap: Record<EnhanceStyle, { task: string; tone?: string }> = {
        ...
        bullet: { task: "bullet" },
        prose: { task: "prose" },
        shorter: { task: "shorter" },
        longer: { task: "longer" },
        headlines: { task: "headlines" },
        eli5: { task: "eli5" },
      };
  ...
  const handleExport = useCallback((format: ExportFormat) => {
    ...
    const { content, filename, mimeType } = exportDoc(docData, format);
    downloadFile(content, filename, mimeType);
    setShowExportMenu(false);
  }, [title, body, mode, checklist]);

  // Handler for copy to clipboard
  const handleCopyToClipboard = useCallback(async () => {
    const textContent = mode === "text"
      ? body
  ...
  // Handle close: finalize doc with attachments/reminders/pin or delete if empty
  const handleClose = useCallback(async () => {
    ...
    // If no doc was created and no content, just close
    if (!createdNoteId && !hasContent) {
      resetState();
      return;
    }

    // If doc was created but now empty, delete it
    if (createdNoteId && !hasContent) {
      try {
        await deleteDoc(createdNoteId);
      } catch (error) {
        console.error("[DocComposer] Failed to delete empty doc:", error);
      }
      resetState();
      return;
    }

    // If no doc was created yet but there's content, create it now
    let docId = createdNoteId;
    if (!docId && hasContent) {
      ...
      try {
        ...
        console.error("[DocComposer] Failed to create doc on close:", error);
    ...
    try {
      ...
      // Users can add attachments by editing the doc after creation
  ...
  const handleCreateNewLabel = useCallback(async () => {
    ...
    try {
      ...
      console.error("[DocComposer] Failed to create label:", error);
    } finally {
      setIsCreatingLabel(false);
    }
  ...
  const colorConfig = DOC_COLORS.find((c) => c.id === color) || DOC_COLORS[0];
  ...
  const docStats = useMemo(() => {
    const textContent = mode === "text"
      ? body
      : checklist.map((item) => item.text).join("\n");
    return getDocStats(textContent);
  }, [mode, body, checklist]);

  const docStatsDisplay = formatDocStats(docStats);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Cmd+Shift+D - Toggle doc date calendar
      ...
      // Cmd+E - Toggle export menu (when there's content)
      if (isMeta && !event.shiftKey && event.key.toLowerCase() === 'e' && hasContent) {
        event.preventDefault();
      ...
      // Cmd+Shift+A - Toggle AI enhance menu (when there's body text)
      if (isMeta && event.shiftKey && event.key.toLowerCase() === 'a' && mode === 'text' && body.trim()) {
        event.preventDefault();
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-10">
                    <div className="flex flex-col items-center gap-3 rounded-2xl bg-zinc-900 px-8 py-5 border border-[var(--color-primary)]/40 shadow-2xl">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-xs text-white/60 mt-0.5">Enhancing your doc</p>
                      </div>
              ...
              <div className="flex items-center gap-1">
                ...
                <div className="relative">
                  <button
                    ...
                    aria-label="Set doc date"
                    title={docDate ? `Date: ${docDate.toLocaleDateString()}` : "Set doc date"}
                  >
                ...
                {hasContent && (
                  <div className="relative">
                    ...
                    {showExportMenu && (
                      <div className="absolute bottom-12 right-0 z-30 min-w-[160px] rounded-xl bg-surface-elevated/95 p-2 shadow-floating backdrop-blur-xl">
                        <button
                          type="button"
                          onClick={() => handleExport("markdown")}
                          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition"
                        >
                ...
                {ENHANCE_STYLES.map((style) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex flex-col items-center gap-1.5 px-2 py-2.5 rounded-xl transition-colors bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 border border-zinc-200 dark:border-zinc-700 group"
                    >
                      <IconComponent className="h-4 w-4 text-zinc-500 dark:text-zinc-400 group-hover:text-[var(--color-primary)] transition-colors" />
                      <span className="text-xs font-medium text-zinc-700 dark:text-zinc-300 text-center leading-tight">{style.label}</span>
                    </button>
                  );
                })}
              </div>
