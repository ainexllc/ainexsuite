...
export function Hint({ hintId, children }: HintProps) {
  ...
  const getArrowStyles = (placement: HintPlacement) => {
    switch (placement) {
      case 'top':
        return 'top-full left-1/2 -translate-x-1/2 border-t-red-500 border-x-transparent border-b-transparent';
      case 'bottom':
        return 'bottom-full left-1/2 -translate-x-1/2 border-b-red-500 border-x-transparent border-t-transparent';
      case 'left':
        return 'left-full top-1/2 -translate-y-1/2 border-l-red-500 border-y-transparent border-r-transparent';
      case 'right':
        return 'right-full top-1/2 -translate-y-1/2 border-r-red-500 border-y-transparent border-l-transparent';
      default:
        return 'bottom-full left-1/2 -translate-x-1/2 border-b-red-500 border-x-transparent border-t-transparent';
    }
  };
  ...
  return (
    <div ref={containerRef} className="relative inline-block">
      ...
      {showHint && (
        ...
        >
          <div className="relative bg-red-500 text-white rounded-lg shadow-lg p-3 max-w-xs">
            <div className={`absolute w-0 h-0 border-[6px] ${getArrowStyles(config.placement)}`} />
            <div className="flex items-start gap-2">
              <div className="flex-1">
                <p className="font-medium text-sm">{config.title}</p>
                <p className="text-xs text-red-100 mt-0.5">{config.description}</p>
              </div>
              <button
