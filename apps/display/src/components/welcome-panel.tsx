...
export function WelcomePanel({ onDismiss }: WelcomePanelProps) {
  return (
    <div className="glass-card rounded-2xl p-6 mb-6 border border-border bg-gradient-to-br from-indigo-500/10 via-transparent to-purple-500/10 relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-foreground">Welcome to Pulse</h2>
              <p className="text-sm text-muted-foreground">Your customizable smart display</p>
            </div>
          </div>
          <button
            onClick={onDismiss}
        ...
        <div className="flex items-center gap-4 p-4 rounded-xl bg-foreground/5 border border-border">
          ...
          <p className="text-sm text-muted-foreground">
            <span className="text-foreground/80 font-medium">Pro tip:</span> Click the <Settings2 className="inline h-4 w-4 mx-1" />
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="p-4 rounded-xl bg-foreground/5 border border-border hover:border-border-hover transition-colors">
