...
import { TaskSpace, TaskList } from '../../types/models';
...
export function SpaceSwitcher() {
  ...
  const handleCreateSpace = async () => {
    const name = prompt('Space Name (e.g., Kitchen Remodel):');
    if (!name || !user) return;
    
    const type = prompt('Type (personal/family/work):') as TaskSpace['type'];
    if (!['personal', 'family', 'work'].includes(type)) return;
