...
import type { Label, LabelDraft } from "@/lib/types/workflow";
...
export async function deleteLabel(userId: string, labelId: string) {
  ...
  // Remove label reference from workflows
  const workflowsWithLabelQuery = query(
    clientWorkflowCollection(userId),
    where("labelIds", "array-contains", labelId),
  );

  const snapshot = await getDocs(workflowsWithLabelQuery);
