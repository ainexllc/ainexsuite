...
export function MemberManager({ isOpen, onClose }: MemberManagerProps) {
  const { getCurrentSpace } = useWorkflowStore();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
          ...
          {isPersonalSpace ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-xs mx-auto">
                This is your personal workflow space. Automations here are private and only visible to you.
              </p>
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-4">
                Select a team or custom space to share your workflows with others.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              {/* Invite button */}
              <button className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed border-zinc-300 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:border-cyan-500 hover:text-cyan-500 transition-colors">
        ...
        <div className="px-6 py-4 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50">
          <button
            onClick={onClose}
            className="w-full py-2.5 rounded-xl bg-cyan-500 text-white font-medium hover:bg-cyan-600 transition-colors"
          >
            Done
          </button>
        </div>
