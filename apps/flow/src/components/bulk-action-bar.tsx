...
// Workflow color options for bulk actions
const WORKFLOW_COLOR_OPTIONS: { value: WorkflowColor; label: string; className: string }[] = [
  { value: "default", label: "Default", className: "bg-zinc-400" },
  { value: "workflow-blue", label: "Blue", className: "bg-blue-500" },
  { value: "workflow-green", label: "Green", className: "bg-emerald-500" },
  { value: "workflow-amber", label: "Amber", className: "bg-amber-500" },
  { value: "workflow-purple", label: "Purple", className: "bg-purple-500" },
  { value: "workflow-pink", label: "Pink", className: "bg-pink-500" },
  { value: "workflow-cyan", label: "Cyan", className: "bg-cyan-500" },
  { value: "workflow-slate", label: "Slate", className: "bg-slate-500" },
];
...
interface BulkActionBarProps {
  ...
  onUnpin: () => void;
  onArchive: () => void;
  onColorChange: (color: WorkflowColor) => void;
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 fade-in duration-200">
      <div className="flex items-center gap-3 px-2 py-2 rounded-full bg-zinc-900/95 dark:bg-zinc-800/95 border border-zinc-700/50 shadow-2xl backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2 pl-2 pr-3">
          ...
          <span className="text-sm font-semibold text-white tabular-nums">{selectedCount}</span>
        </div>
            ...
            {showColorPicker && (
              <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 p-3 rounded-2xl bg-zinc-900/95 border border-zinc-700/50 shadow-2xl backdrop-blur-xl">
                <div className="grid grid-cols-4 gap-2">
                  {WORKFLOW_COLOR_OPTIONS.map((color) => (
                    <button
                      key={color.value}
                      onClick={() => {
