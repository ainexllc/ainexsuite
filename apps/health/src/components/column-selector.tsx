...
type ColumnSelectorProps = {
  section: "today" | "history";
};

export function ColumnSelector({ section }: ColumnSelectorProps) {
  const { preferences, updatePreferences } = usePreferences();

  const currentColumns = section === "today"
    ? preferences.todayColumns
    : preferences.historyColumns;

  const handleColumnChange = (cols: MasonryColumns) => {
    if (section === "today") {
      updatePreferences({ todayColumns: cols });
    } else {
      updatePreferences({ historyColumns: cols });
    }
  };
