    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 border border-outline-subtle rounded-lg bg-surface-elevated text-ink-900 focus:outline-none focus:ring-2 focus:ring-emerald-500"
        />
        {loading && (
      ...
      {selectedFood && (
        <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="font-medium text-ink-900">{selectedFood.name}</span>
            <button
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="w-20 px-2 py-1 border border-outline-subtle rounded text-center"
            />
            ...
            <button
              onClick={handleAdd}
              className="ml-auto flex items-center gap-1 px-3 py-1 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors"
            >
      ...
      <div className="max-h-64 overflow-y-auto space-y-1">
        {query.length < 2 && recentFoods.length > 0 && (
          <p className="text-xs text-ink-400 mb-2">Recent foods</p>
        )}
        ...
        {query.length >= 2 && results.length === 0 && !loading && (
          <div className="text-center py-8">
            <p className="text-ink-500 mb-3">No foods found for &quot;{query}&quot;</p>
            {onCreateCustom && (
              <button
                onClick={onCreateCustom}
                className="inline-flex items-center gap-2 px-4 py-2 border border-outline-subtle rounded-lg text-ink-700 hover:bg-surface-subtle transition-colors"
              >
                <Plus className="w-4 h-4" />
                Create Custom Food
              </button>
