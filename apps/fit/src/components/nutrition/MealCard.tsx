...
export function MealCard({ meal, onEdit, onDelete }: MealCardProps) {
  ...
  return (
    <div className="p-4 bg-background/60 border border-border rounded-xl backdrop-blur-xl">
        ...
        <div className="relative">
          ...
          {showMenu && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 py-1 bg-background border border-border rounded-lg shadow-lg z-20 min-w-[120px]">
                {onEdit && (
                  <button
                    onClick={() => {
                      onEdit(meal);
      ...
      <div className="space-y-2 mb-3">
        {meal.foods.map((entry, index) => (
          <div key={index} className="flex items-center justify-between text-sm">
            <span className="text-foreground">{entry.food.name}</span>
      ...
      {/* Notes */}
      {meal.notes && (
        <p className="mt-2 text-xs text-muted-foreground italic">{meal.notes}</p>
      )}
    </div>
  );
}
