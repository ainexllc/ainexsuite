...
export function AIAssistant() {
  ...
  const { messages, sendMessage, loading } = useGrokAssistant({
    appName: 'fit',
    systemPrompt: `You are an expert fitness and training AI coach.

Your role is to help users:
- Design effective workout programs
- Optimize exercise form and technique
- Create progressive training plans
- Provide nutrition and recovery advice
- Track progress and adjust programs
- Prevent injuries through proper planning
- Stay motivated and consistent

Be knowledgeable, supportive, and focused on evidence-based training principles.`,
  });
  ...
  return (
    <>
      <button
        ...
        className={cn(
          'fixed bottom-8 right-8 w-14 h-14 rounded-full bg-gradient-to-br from-orange-500 to-red-500 shadow-lg hover:shadow-xl transition-all hover:scale-110 flex items-center justify-center z-50',
          isOpen && 'rotate-180'
        )}
      >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 ? (
              <div className="text-center text-ink-600 py-8 space-y-2 text-sm">
                <p>I&#39;m here to help you reach your fitness goals!</p>
                <p>Ask me about:</p>
