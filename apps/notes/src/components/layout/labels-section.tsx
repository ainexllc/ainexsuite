...
export function LabelsSection({ onClose }: LabelsSectionProps) {
  ...
  const { activeLabelIds, setActiveLabelIds, notes } = useNotes();
  ...
  return (
    <NavigationSection title="Labels">
      <div className="space-y-1">
        {labelsLoading ? (
          ...
          labelNodes.map(({ label, depth }) => {
            ...
            const noteCount = noteCounts.get(label.id) || 0;
      ...
      <ConfirmationDialog
        ...
        description={`This will remove the tag from ${noteCounts.get(deletingLabel?.id || "") || 0} note${(noteCounts.get(deletingLabel?.id || "") || 0) === 1 ? "" : "s"}. This action cannot be undone.`}
