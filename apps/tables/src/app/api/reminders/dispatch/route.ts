...
import { adminTableDoc, adminPreferenceDoc } from "@/lib/firebase/admin-collections";
...
async function fetchDueReminders(now: Date) {
  ...
  for (const docSnapshot of [...scheduledSnapshot.docs, ...snoozedSnapshot.docs]) {
    docs.set(docSnapshot.ref.path, docSnapshot as QueryDocumentSnapshot<ReminderDoc>);
  }
...
export async function POST(request: NextRequest) {
  ...
  for (const snapshot of dueSnapshots) {
    ...
    const subject = `Reminder: ${data.titleSnapshot || "Untitled doc"}`;
    ...
    if (nextOccurrence && data.tableId) {
      try {
        await adminTableDoc(userId, data.tableId).update({
