...
import { adminTableDoc } from "@/lib/firebase/admin-collections";
...
export async function POST(request: NextRequest) {
  ...
  const docRef = adminTableDoc(user.uid, tableId);
  ...
  if (collaboratorRecord.uid === user.uid) {
    return NextResponse.json({ error: "You already own this table" }, { status: 400 });
  }
...
export async function DELETE(request: NextRequest) {
  ...
  const remainingIds = collaboratorUserId
    ? (docData.sharedWithUserIds ?? []).filter((id: string) => id !== collaboratorUserId)
    : docData.sharedWithUserIds ?? [];
