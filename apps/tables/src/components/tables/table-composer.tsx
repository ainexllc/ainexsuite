...
import type { ChecklistItem, TableColor, TablePriority, TableType } from "@/lib/types/table";
import { createEmptySpreadsheet } from "@/lib/types/spreadsheet";
...
import { InlineCalculator } from "@/components/tables/inline-calculator";
...
export function TableComposer({ placeholder = 'Create a new table...' }: TableComposerProps) {
  ...
  const [mode, setMode] = useState<TableType>("text");
  const [checklist, setChecklist] = useState<ChecklistItem[]>([]);
  const [color, setColor] = useState<TableColor>("default");
  const [pinned, setPinned] = useState(false);
  const [priority, setPriority] = useState<TablePriority>(null);
  ...
  const applyTemplate = useCallback(async (template: NoteTemplate) => {
    // For spreadsheet template, create immediately and redirect
    if (template.mode === "spreadsheet") {
      const newSpreadsheet = createEmptySpreadsheet(100, 26);
      const newId = await createTable({
        title: template.title || "Untitled Spreadsheet",
        body: "",
        type: "spreadsheet",
        color: "default",
        checklist: [],
        ...
        spaceId: currentSpace?.id !== "personal" ? currentSpace?.id : undefined,
      });
  ...
  useEffect(() => {
    ...
    const createDraftNote = async () => {
      ...
      try {
        ...
        if (tableId) {
          setCreatedNoteId(tableId);
        }
      } catch (error) {
        console.error("[TableComposer] Failed to auto-create doc:", error);
      } finally {
        isCreatingRef.current = false;
      }
    };
  ...
  }, [expanded, createdNoteId, title, body, mode, checklist, color, createTable]);
  ...
  useEffect(() => {
    ...
    autoSaveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        console.error("[TableComposer] Auto-save failed:", error);
      }
    }, 1500);
  ...
  }, [createdNoteId, title, body, mode, checklist, color, selectedLabelIds, updateDoc]);
  ...
  const handleClose = useCallback(async () => {
    ...
    if (createdNoteId && !hasContent) {
      try {
        await deleteTable(createdNoteId);
      } catch (error) {
        console.error("[TableComposer] Failed to delete empty doc:", error);
      }
      resetState();
      return;
    }
    ...
    let tableId = createdNoteId;
    if (!tableId && hasContent) {
      ...
      try {
        ...
        console.error("[TableComposer] Failed to create doc on close:", error);
    ...
    try {
      ...
      if (Object.keys(updates).length > 0) {
        await updateDoc(tableId, updates);
      }
  ...
  const handleCreateNewLabel = useCallback(async () => {
    ...
    try {
      ...
      console.error("[TableComposer] Failed to create label:", error);
    } finally {
      setIsCreatingLabel(false);
    }
  ...
  const colorConfig = TABLE_COLORS.find((c) => c.id === color) || TABLE_COLORS[0];
  ...
  // For Tables app, clicking the composer should create a spreadsheet directly
  const handleCreateSpreadsheet = useCallback(async () => {
    const spreadsheetTemplate = DOC_TEMPLATES.find(t => t.id === "spreadsheet");
    if (spreadsheetTemplate) {
      await applyTemplate(spreadsheetTemplate);
    }
  }, [applyTemplate]);
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-zinc-200 dark:border-zinc-800">
              <div className="flex items-center gap-1">
                ...
                <button
                  type="button"
                  className="p-2 rounded-full transition-colors text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800"
                  onClick={() => applyTemplate(DOC_TEMPLATES.find(t => t.id === "spreadsheet")!)}
