...
const INSIGHTS_STORAGE_KEY = "docs-insights-collapsed";
...
export function WorkspaceInsights({ className }: WorkspaceInsightsProps) {
  ...
  // Only analyze if we have enough tables
  const RECENT_COUNT = 5;
  const recentNotes = tables
  ...
  const STORAGE_KEY = `ainex-docs-workspace-insights-${currentSpaceId}`;
  ...
  useEffect(() => {
    if (docsLoading || isCollapsed) return;
  ...
  }, [docsLoading, hasEnoughData, currentSpaceId, isCollapsed, STORAGE_KEY]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-text-primary truncate">
                Create 2+ docs to unlock AI Insights
              </p>
            </div>
          </div>
        </div>
