...
function getPreview(table: Table): string {
  if (table.body?.trim()) {
    return table.body.trim().slice(0, 160);
  }
  if (table.checklist?.length) {
    const first = table.checklist.find((item: { text: string }) => item.text.trim());
...
export function TrashBoard() {
  const { trashed, loading, restoreTable, destroyTable, destroyAllTables } = useTables();
  ...
  const grouped = useMemo(() => {
    return trashed.map(table => ({
      ...table,
      deletedLabel: table.deletedAt
    ...
    <Container className="space-y-8 lg:px-0" variant="narrow">
      <header className="space-y-3">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-ink-800">Review deleted tables</h1>
            <p className="text-sm text-muted">
              Tables stay here for 30 days before they&apos;re permanently deleted. Restore mistakes or remove them forever.
            </p>
          </div>
          {hasNotes ? (
            <button
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {grouped.map(table => {
            const isRestoring = pendingRestoreId === table.id;
            const isDeleting = pendingDeleteId === table.id;
              ...
              >
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-sm font-semibold text-ink-800">
                    {table.title?.trim() ? table.title : "Untitled table"}
                    <span className="text-xs font-medium uppercase tracking-wide text-ink-400">
                      Deleted {table.deletedLabel}
                    </span>
                  </div>
                  {table.preview ? (
                    <p className="text-sm text-muted">{table.preview}</p>
                  ) : null}
                </div>
        ...
        <EmptyState
          ...
          description="Delete tables from your workspace and they'll collect here for 30 days before being permanently removed."
          icon={Trash2}
          variant="default"
        />
      )}
      ...
      <ConfirmationDialog
        ...
        onConfirm={async () => {
          ...
          try {
            await destroyTable(confirmDeleteId);
            setConfirmDeleteId(null);
          } finally {
      ...
      <ConfirmationDialog
        ...
        title="Empty trash?"
        description="This will permanently delete all tables in the trash, including their attachments. This action cannot be undone."
