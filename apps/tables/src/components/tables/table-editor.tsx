...
import type { BackgroundOverlay } from "@/lib/types/table";
...
export function TableEditor({ table, onClose }: TableEditorProps) {
  ...
  const [body, setBody] = useState(table.body);
  const [mode, setMode] = useState<"text" | "checklist">(
    table.type === "checklist" ? "checklist" : "text",
  );
  const [checklist, setChecklist] = useState<ChecklistItem[]>(
    table.checklist,
  );
  const [color, setColor] = useState<TableColor>(table.color);
  const [backgroundImage, setBackgroundImage] = useState<string | null>(table.backgroundImage ?? null);
  const [backgroundOverlay, setBackgroundOverlay] = useState<BackgroundOverlay>(table.backgroundOverlay ?? 'auto');
  ...
  const [pinned, setPinned] = useState(table.pinned);
  const [priority, setPriority] = useState<TablePriority>(table.priority ?? null);
  const [showPriorityPicker, setShowPriorityPicker] = useState(false);
  const [existingAttachments, setExistingAttachments] = useState<TableAttachment[]>(
    table.attachments,
  );
  const [removedAttachments, setRemovedAttachments] = useState<TableAttachment[]>([]);
  ...
  const lastSavedBodyRef = useRef(table.body);
  ...
  const [reminderEnabled, setReminderEnabled] = useState(Boolean(table.reminderAt));
  const [reminderValue, setReminderValue] = useState(
    table.reminderAt ? formatDateTimeLocalInput(table.reminderAt) : "",
  );
  ...
  const currentColorConfig = useMemo(() => {
    return TABLE_COLORS.find((c) => c.id === color) || TABLE_COLORS[0];
  }, [color]);
  ...
  const [selectedSpaceId, setSelectedSpaceId] = useState<string>(table.spaceId || "personal");
  ...
  const handleAutoSave = useCallback(async (data: typeof autoSaveData) => {
    const updates: TableDraft = {};

    if (data.title !== table.title) updates.title = data.title;
    if (data.body !== table.body) updates.body = data.body;
    if (data.type !== table.type) {
      updates.checklist = data.checklist;
      updates.body = data.body;
    } else if (data.type === "checklist") {
      const checklistChanged = JSON.stringify(data.checklist) !== JSON.stringify(table.checklist);
      if (checklistChanged) updates.checklist = data.checklist;
    }
    if (data.color !== table.color) updates.color = data.color;
    if (JSON.stringify(data.labelIds) !== JSON.stringify(table.labelIds ?? [])) {
      updates.labelIds = data.labelIds;
    }
    const tableSpaceId = table.spaceId || undefined;
    if (data.spaceId !== tableSpaceId) updates.spaceId = data.spaceId;

    if (Object.keys(updates).length > 0) {
      await updateDoc(table.id, updates);
    }
  }, [table, updateDoc]);
  ...
  const handleBackgroundChange = useCallback(async (newBackgroundImage: string | null) => {
    ...
    await updateDoc(table.id, { backgroundImage: newBackgroundImage });
  }, [table.id, updateDoc]);

  const handleOverlayChange = useCallback(async (newOverlay: BackgroundOverlay) => {
    setBackgroundOverlay(newOverlay);
    // Auto-save to database
    await updateDoc(table.id, { backgroundOverlay: newOverlay });
  }, [table.id, updateDoc]);
  ...
  const existingReminder = useMemo(() => {
    ...
    return reminders.find((item) => item.id === table.reminderId) ?? null;
  }, [reminders, table.reminderId]);
  ...
  const handleFinalizeAndClose = useCallback(async () => {
    ...
    // Handle pin changes
    const pinnedChanged = pinned !== table.pinned;

    if (pinnedChanged) {
      await togglePin(table.id, pinned);
    }

    // Handle priority changes
    const priorityChanged = priority !== (table.priority ?? null);
    if (priorityChanged) {
      await updateDoc(table.id, { priority });
    }
    ...
    if (removedAttachments.length > 0) {
      await Promise.all(
        removedAttachments.map((attachment) =>
          removeAttachment(table.id, attachment),
        ),
      );
    }
    ...
    const shouldRemoveReminder = !reminderEnabled && Boolean(table.reminderId);
    const shouldCreateReminder = reminderEnabled && !table.reminderId && reminderFireAt;
    const shouldUpdateExisting = reminderEnabled && Boolean(table.reminderId) && Boolean(reminderFireAt);
    const reminderTimeChanged =
      reminderEnabled && reminderFireAt &&
      (!table.reminderAt || table.reminderAt.getTime() !== reminderFireAt.getTime());
    ...
    const reminderFrequencyChanged =
      reminderEnabled && existingReminder
        ...
        : reminderEnabled && !table.reminderId && reminderFrequency !== "once";
    ...
    if (shouldRemoveReminder && table.reminderId) {
      await deleteReminder(table.reminderId);
      await updateDoc(table.id, { reminderAt: null, reminderId: null });
    }
    ...
    if (shouldCreateReminder && reminderFireAt) {
      ...
      await updateReminder(table.reminderId, {
        fireAt: reminderFireAt,
        channels: activeChannels,
        frequency: reminderFrequency,
        customCron: reminderFrequency === "custom" ? customCron || null : null,
  ...
  useEffect(() => {
    if (!table.reminderId && !reminderEnabled) {
      setReminderChannels([...preferences.reminderChannels]);
    }
  }, [preferences.reminderChannels, table.reminderId, reminderEnabled]);
  ...
  const handleRemoveExistingAttachment = (attachment: TableAttachment) => {
            ...
            <div className="flex items-center gap-2">
              ...
              <div className={clsx(
                ...
                <button
                  type="button"
                  onClick={async () => {
                    const newNoteId = await duplicateTable(table.id);
                    if (newNoteId) {
                      onClose();
                    }
                  }}
                ...
                <div className="absolute inset-0 flex items-center justify-center z-10">
                  <div className="flex flex-col items-center gap-4 rounded-2xl bg-zinc-900 px-10 py-6 border border-[var(--color-primary)]/40 shadow-2xl">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-sm text-white/60 mt-1">Enhancing your entire doc</p>
                    </div>
