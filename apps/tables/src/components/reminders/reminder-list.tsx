...
export function ReminderList() {
  ...
  const { tables, updateDoc } = useTables();
  const [processingId, setProcessingId] = useState<string | null>(null);

  const entries = useMemo(() => {
    return reminders.map((reminder) => {
      const table = tables.find((item) => item.id === reminder.tableId) ?? null;
  ...
  const handleSnooze = async (reminderId: string, minutes: number) => {
    ...
    try {
      ...
      await updateDoc(entry.reminder.tableId, {
        reminderAt: newDate,
      });
    } finally {
  ...
  if (!entries.length) {
    return (
      <EmptyState
        ...
        description="Set a reminder from any doc to have it appear here."
        icon={AlarmClock}
        variant="default"
      />
    );
    ...
    <ListSection title={label} count={items.length}>
      {items.length ? (
        <ul className="space-y-3">
          {items.map(({ reminder, table, timestamp, title }) => {
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted">
                      {table?.labelIds?.length ? `${table.labelIds.length} labels · ` : ""}
