...
import type { Label, LabelDraft } from "@/lib/types/table";
import { clientTableCollection } from "@/lib/firebase/client-collections";
...
export function subscribeToLabels(
  ...
  return onSnapshot(labelsQuery, (snapshot) => {
    handler(snapshot.docs.map(docSnapshot => docSnapshot.data()));
  });
}
...
export async function deleteLabel(userId: string, labelId: string) {
  ...
  // Remove label reference from tables.
  const tablesWithLabelQuery = query(
    clientTableCollection(userId),
    where("labelIds", "array-contains", labelId),
  );

  const snapshot = await getDocs(tablesWithLabelQuery);
