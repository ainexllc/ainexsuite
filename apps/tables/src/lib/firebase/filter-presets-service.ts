 ...
 */
function toFilterPreset(id: string, docData: FilterPresetDoc): FilterPreset {
  return {
    id,
    name: docData.name,
    filters: toRuntimeFilters(docData.filters),
    sort: docData.sort,
...
export function subscribeToFilterPresets(
  ...
  return onSnapshot(presetsQuery, (snapshot) => {
    const presets = snapshot.docs.map(docSnapshot =>
      toFilterPreset(docSnapshot.id, docSnapshot.data() as FilterPresetDoc)
    );
    handler(presets);
  });
}
