...
/**
 * Get fallback backgrounds (for when dynamic loading fails)
 * @deprecated Use useBackgrounds hook instead for dynamic backgrounds
 */
...
export function getTextColorClasses(
  ...
  variant: 'title' | 'body' | 'muted' | 'checklist' | 'checklist-completed'
): string {
  if (!bg) {
...
/**
 * Find a background by ID in multiple lists (for combining dynamic + fallback)
 */
export function findBackgroundById(
  id: string,
  ...lists: BackgroundOption[][]
): BackgroundOption | undefined {
  for (const list of lists) {
    const found = list.find(bg => bg.id === id);
