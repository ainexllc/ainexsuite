...
import type { JournalSpace, JournalSpaceDraft, SpaceType } from "@/lib/types/space";
import {
  subscribeToSpaces,
...
const DEFAULT_PERSONAL_SPACE: JournalSpace = {
  id: "personal",
  name: "My Journal",
  type: "personal",
  members: [],
...
type SpacesContextValue = {
  spaces: JournalSpace[];
  currentSpace: JournalSpace;
  currentSpaceId: string;
  loading: boolean;
  setCurrentSpace: (spaceId: string) => void;
  createSpace: (input: { name: string; type: SpaceType }) => Promise<string>;
  updateSpace: (spaceId: string, updates: JournalSpaceDraft) => Promise<void>;
...
export function SpacesProvider({ children }: SpacesProviderProps) {
  const { user } = useAuth();
  const [userSpaces, setUserSpaces] = useState<JournalSpace[]>([]);
  ...
  const spaces = useMemo(() => {
    const personalSpace: JournalSpace = {
  ...
  const handleSetCurrentSpace = useCallback((spaceId: string) => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("journey-current-space", spaceId);
    }
  }, []);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedSpaceId = localStorage.getItem("journey-current-space");
      if (savedSpaceId) {
  ...
  const handleUpdateSpace = useCallback(
    async (spaceId: string, updates: JournalSpaceDraft) => {
