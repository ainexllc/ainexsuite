"use client";

import { useCallback, useState, useEffect, useRef } from "react";
import type { Editor } from "@tiptap/react";
import {
  Bold,
  Italic,
  Underline,
  Strikethrough,
  Code,
  Heading1,
  Heading2,
  Heading3,
  List,
  ListOrdered,
  Quote,
  Type,
  Undo2,
  Redo2,
  Minus,
  ChevronDown,
  ImageIcon,
} from "lucide-react";
import { cn } from "../../lib/utils";

/**
 * Font color options for the text color picker
 */
export const FONT_COLORS = [
  { name: "Red", color: "#ef4444" },
  { name: "Orange", color: "#f97316" },
  { name: "Amber", color: "#f59e0b" },
  { name: "Green", color: "#22c55e" },
  { name: "Blue", color: "#3b82f6" },
  { name: "Purple", color: "#8b5cf6" },
  { name: "Pink", color: "#ec4899" },
  { name: "Gray", color: "#6b7280" },
] as const;

export type FontColor = (typeof FONT_COLORS)[number];

/**
 * Helper to execute editor commands safely
 * Prevents focus loss and maintains selection state
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const runCommand = (editor: Editor, fn: (chain: any) => any): void => {
  const chain = editor.chain().focus();
  fn(chain).run();
};

export interface EditorToolbarProps {
  /** TipTap editor instance */
  editor: Editor;
  /** Additional CSS classes */
  className?: string;
  /** Callback when image button is clicked */
  onImageClick?: () => void;
  /** Force light text/icons (for dark backgrounds) */
  forceLightText?: boolean;
  /** Force dark text/icons (for light backgrounds) */
  forceDarkText?: boolean;
  /** Background brightness hint */
  backgroundBrightness?: "light" | "dark";
  /** Whether the entry has a cover image */
  hasCover?: boolean;
  /** Show headings buttons */
  showHeadings?: boolean;
  /** Show lists buttons */
  showLists?: boolean;
  /** Show block elements (quote, hr) */
  showBlockElements?: boolean;
  /** Show undo/redo buttons */
  showUndoRedo?: boolean;
  /** Show text color picker */
  showTextColor?: boolean;
  /** Show image button (requires onImageClick) */
  showImageButton?: boolean;
  /** Custom font colors array */
  fontColors?: readonly FontColor[];
}

export interface ToolbarButtonProps {
  onClick: () => void;
  isActive?: boolean;
  disabled?: boolean;
  title: string;
  children: React.ReactNode;
  onDarkBackground?: boolean;
}

/**
 * ToolbarButton - Individual toolbar button with active state
 */
export function ToolbarButton({
  onClick,
  isActive,
  disabled,
  title,
  children,
  onDarkBackground,
}: ToolbarButtonProps) {
  return (
    <button
      type="button"
      onMouseDown={(e) => {
        e.preventDefault(); // Prevent focus loss - keeps text selected
        e.stopPropagation();
        if (!disabled) {
          onClick();
        }
      }}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
      }}
      disabled={disabled}
      title={title}
      className={cn(
        "p-1.5 rounded-md transition-colors",
        isActive
          ? onDarkBackground
            ? "bg-white/25 text-white"
            : "bg-black/15 dark:bg-white/20 text-zinc-900 dark:text-white"
          : onDarkBackground
            ? "text-white/75 hover:text-white hover:bg-white/15"
            : "text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white hover:bg-black/10 dark:hover:bg-white/15",
        disabled && "opacity-50 cursor-not-allowed"
      )}
    >
      {children}
    </button>
  );
}

/**
 * ToolbarDivider - Visual separator between button groups
 */
export function ToolbarDivider({
  onDarkBackground,
}: {
  onDarkBackground?: boolean;
}) {
  return (
    <div
      className={cn(
        "w-px h-5 mx-1",
        onDarkBackground
          ? "bg-white/25"
          : "bg-zinc-400/40 dark:bg-zinc-500/50"
      )}
    />
  );
}

/**
 * EditorToolbar - Full-featured toolbar for TipTap editor
 *
 * Features:
 * - Text formatting (bold, italic, underline, strikethrough, code)
 * - Text color picker with customizable colors
 * - Headings (H1, H2, H3)
 * - Lists (bullet, numbered)
 * - Block elements (quote, horizontal rule)
 * - Undo/Redo
 * - Image insertion (with callback)
 * - Adaptive styling for dark/light backgrounds
 *
 * @example
 * ```tsx
 * <EditorToolbar
 *   editor={editor}
 *   onImageClick={() => openImagePicker()}
 *   showHeadings={true}
 *   showLists={true}
 * />
 * ```
 */
export function EditorToolbar({
  editor,
  className,
  onImageClick,
  forceLightText,
  forceDarkText: _forceDarkText,
  backgroundBrightness,
  hasCover,
  showHeadings = true,
  showLists = true,
  showBlockElements = true,
  showUndoRedo = true,
  showTextColor = true,
  showImageButton = true,
  fontColors = FONT_COLORS,
}: EditorToolbarProps) {
  const [showFontColors, setShowFontColors] = useState(false);
  const fontColorRef = useRef<HTMLDivElement>(null);

  // Determine if we're on a dark background (needs light icons/text)
  const onDarkBackground =
    forceLightText || backgroundBrightness === "dark" || hasCover;

  // Close font color picker when clicking outside
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        fontColorRef.current &&
        !fontColorRef.current.contains(event.target as Node)
      ) {
        setShowFontColors(false);
      }
    };
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, []);

  const setFontColor = useCallback(
    (color: string | null) => {
      if (color === null) {
        runCommand(editor, (c) => c.unsetColor());
      } else {
        runCommand(editor, (c) => c.setColor(color));
      }
      setShowFontColors(false);
    },
    [editor]
  );

  return (
    <div
      className={cn(
        "flex items-center gap-0.5 flex-wrap p-2 border-b rounded-t-lg backdrop-blur-sm",
        onDarkBackground
          ? "bg-black/50 dark:bg-black/60 border-white/15 dark:border-white/20"
          : "bg-white/70 dark:bg-zinc-900/80 border-black/10 dark:border-white/15",
        className
      )}
    >
      {/* Text formatting */}
      <ToolbarButton
        onClick={() => runCommand(editor, (c) => c.toggleBold())}
        isActive={editor.isActive("bold")}
        title="Bold (Cmd+B)"
        onDarkBackground={onDarkBackground}
      >
        <Bold className="h-4 w-4" />
      </ToolbarButton>

      <ToolbarButton
        onClick={() => runCommand(editor, (c) => c.toggleItalic())}
        isActive={editor.isActive("italic")}
        title="Italic (Cmd+I)"
        onDarkBackground={onDarkBackground}
      >
        <Italic className="h-4 w-4" />
      </ToolbarButton>

      <ToolbarButton
        onClick={() => runCommand(editor, (c) => c.toggleUnderline())}
        isActive={editor.isActive("underline")}
        title="Underline (Cmd+U)"
        onDarkBackground={onDarkBackground}
      >
        <Underline className="h-4 w-4" />
      </ToolbarButton>

      <ToolbarButton
        onClick={() => runCommand(editor, (c) => c.toggleStrike())}
        isActive={editor.isActive("strike")}
        title="Strikethrough"
        onDarkBackground={onDarkBackground}
      >
        <Strikethrough className="h-4 w-4" />
      </ToolbarButton>

      {/* Font color picker */}
      {showTextColor && (
        <div className="relative" ref={fontColorRef}>
          <button
            type="button"
            onMouseDown={(e) => {
              e.preventDefault();
              setShowFontColors(!showFontColors);
            }}
            title="Text color"
            className={cn(
              "flex items-center gap-0.5 p-1.5 rounded-md transition-colors",
              editor.getAttributes("textStyle").color
                ? "bg-[var(--color-primary)]/20 text-[var(--color-primary)]"
                : onDarkBackground
                  ? "text-white/75 hover:text-white hover:bg-white/15"
                  : "text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white hover:bg-black/10 dark:hover:bg-white/15"
            )}
          >
            <Type className="h-4 w-4" />
            <ChevronDown className="h-3 w-3" />
          </button>

          {showFontColors && (
            <div className="absolute top-full left-0 mt-1 z-50 p-2 bg-surface border border-border rounded-lg shadow-lg">
              <div className="grid grid-cols-4 gap-1.5">
                {fontColors.map((item) => (
                  <button
                    key={item.name}
                    type="button"
                    onMouseDown={(e) => {
                      e.preventDefault();
                      setFontColor(item.color);
                    }}
                    title={item.name}
                    className="w-6 h-6 rounded border border-border hover:scale-110 transition-transform flex items-center justify-center"
                    style={{ backgroundColor: item.color }}
                  >
                    <span className="text-white text-[10px] font-bold drop-shadow">
                      A
                    </span>
                  </button>
                ))}
              </div>
              {editor.getAttributes("textStyle").color && (
                <button
                  type="button"
                  onMouseDown={(e) => {
                    e.preventDefault();
                    setFontColor(null);
                  }}
                  className="w-full mt-2 px-2 py-1 text-xs text-muted-foreground hover:text-foreground bg-surface-muted rounded"
                >
                  Default
                </button>
              )}
            </div>
          )}
        </div>
      )}

      <ToolbarButton
        onClick={() => runCommand(editor, (c) => c.toggleCode())}
        isActive={editor.isActive("code")}
        title="Inline code"
        onDarkBackground={onDarkBackground}
      >
        <Code className="h-4 w-4" />
      </ToolbarButton>

      {/* Headings */}
      {showHeadings && (
        <>
          <ToolbarDivider onDarkBackground={onDarkBackground} />

          <ToolbarButton
            onClick={() => {
              editor.chain().focus().toggleHeading({ level: 1 }).run();
            }}
            isActive={editor.isActive("heading", { level: 1 })}
            title="Heading 1"
            onDarkBackground={onDarkBackground}
          >
            <Heading1 className="h-4 w-4" />
          </ToolbarButton>

          <ToolbarButton
            onClick={() => {
              editor.chain().focus().toggleHeading({ level: 2 }).run();
            }}
            isActive={editor.isActive("heading", { level: 2 })}
            title="Heading 2"
            onDarkBackground={onDarkBackground}
          >
            <Heading2 className="h-4 w-4" />
          </ToolbarButton>

          <ToolbarButton
            onClick={() => {
              editor.chain().focus().toggleHeading({ level: 3 }).run();
            }}
            isActive={editor.isActive("heading", { level: 3 })}
            title="Heading 3"
            onDarkBackground={onDarkBackground}
          >
            <Heading3 className="h-4 w-4" />
          </ToolbarButton>
        </>
      )}

      {/* Lists */}
      {showLists && (
        <>
          <ToolbarDivider onDarkBackground={onDarkBackground} />

          <ToolbarButton
            onClick={() => runCommand(editor, (c) => c.toggleBulletList())}
            isActive={editor.isActive("bulletList")}
            title="Bullet list"
            onDarkBackground={onDarkBackground}
          >
            <List className="h-4 w-4" />
          </ToolbarButton>

          <ToolbarButton
            onClick={() => runCommand(editor, (c) => c.toggleOrderedList())}
            isActive={editor.isActive("orderedList")}
            title="Numbered list"
            onDarkBackground={onDarkBackground}
          >
            <ListOrdered className="h-4 w-4" />
          </ToolbarButton>
        </>
      )}

      {/* Block elements */}
      {showBlockElements && (
        <>
          <ToolbarDivider onDarkBackground={onDarkBackground} />

          <ToolbarButton
            onClick={() => runCommand(editor, (c) => c.toggleBlockquote())}
            isActive={editor.isActive("blockquote")}
            title="Quote"
            onDarkBackground={onDarkBackground}
          >
            <Quote className="h-4 w-4" />
          </ToolbarButton>

          <ToolbarButton
            onClick={() => runCommand(editor, (c) => c.setHorizontalRule())}
            title="Horizontal rule"
            onDarkBackground={onDarkBackground}
          >
            <Minus className="h-4 w-4" />
          </ToolbarButton>
        </>
      )}

      {/* Image */}
      {showImageButton && onImageClick && (
        <ToolbarButton
          onClick={onImageClick}
          title="Add image"
          onDarkBackground={onDarkBackground}
        >
          <ImageIcon className="h-4 w-4" />
        </ToolbarButton>
      )}

      {/* Undo/Redo */}
      {showUndoRedo && (
        <>
          <ToolbarDivider onDarkBackground={onDarkBackground} />

          <ToolbarButton
            onClick={() => runCommand(editor, (c) => c.undo())}
            disabled={!editor.can().undo()}
            title="Undo (Cmd+Z)"
            onDarkBackground={onDarkBackground}
          >
            <Undo2 className="h-4 w-4" />
          </ToolbarButton>

          <ToolbarButton
            onClick={() => runCommand(editor, (c) => c.redo())}
            disabled={!editor.can().redo()}
            title="Redo (Cmd+Shift+Z)"
            onDarkBackground={onDarkBackground}
          >
            <Redo2 className="h-4 w-4" />
          </ToolbarButton>
        </>
      )}
    </div>
  );
}

export default EditorToolbar;
