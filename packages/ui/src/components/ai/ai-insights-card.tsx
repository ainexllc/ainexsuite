      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2" style={{ color: accentColor }}>
            ...
            <h3 className={cn("font-semibold text-white", isSidebar ? "text-sm" : "text-base")}>
              {title}
            </h3>
          ...
          <div className="flex items-center gap-1">
            {onRefresh && (
              <button
                ...
                className="p-2 text-white/40 hover:text-white hover:bg-white/10 rounded-full transition-colors disabled:opacity-50"
        ...
        {isLoading ? (
          <div
            className={cn(
              "flex flex-col items-center justify-center text-white/50 gap-3",
              isSidebar ? "py-6" : "py-8"
            )}
          >
            <Loader2
          ...
          <div className={cn("text-center space-y-3", isSidebar ? "py-6" : "py-8")}>
            <div className="text-yellow-400 text-sm font-medium">Insights Unavailable</div>
            <div className="text-white/60 text-xs leading-relaxed max-w-sm mx-auto">{error}</div>
          </div>
        ) : (
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {tags.map((tag, i) => (
        <span
          key={i}
          className="inline-flex items-center rounded-md bg-white/10 px-2.5 py-1 text-xs text-white/80 ring-1 ring-inset ring-white/10"
        >
          {tag}
        </span>
      ))}
