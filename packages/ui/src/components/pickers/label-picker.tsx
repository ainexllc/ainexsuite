...
export interface LabelPickerProps {
  ...
  labels: PickerLabel[];
  /** Function to create a new label (optional) */
  onCreateLabel?: (name: string) => Promise<string | null>;
  /** Whether labels can be created */
  canCreate?: boolean;
  ...
  /** Empty state text when no labels */
  emptyText?: string;
...
/**
 * LabelPicker - Shared label/tag picker for selecting and creating labels
 *
 * Features:
...
export function LabelPicker({
  ...
  emptyText = "No tags yet. Type above to create your first tag.",
  noMatchText,
  className,
}: LabelPickerProps) {
  ...
  const handleCreateNewLabel = useCallback(async () => {
    ...
    if (!name || isCreatingLabel || !onCreateLabel) {
      return;
    }
    ...
    try {
      ...
      const newLabelId = await onCreateLabel(name);
      if (newLabelId) {
  ...
  const getDotStyles = (labelColor: string | undefined, isSelected: boolean) => {
  ...
  const defaultNoMatchText = `No matching tags. ${
    canCreate && onCreateLabel
      ? `Press Enter or click Create to add "${newLabelName.trim()}".`
      : ""
  }`;

  return (
    <div className={cn("space-y-3", !compact && "pt-3 mt-3", className)}>
      ...
      >
        {/* Search/Create input */}
        <div className={cn("flex items-center gap-2", hasBackground && "mb-3")}>
          <div className="relative flex-1">
            ...
            {isCreatingLabel && (
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                <Loader2
                  className={cn("h-4 w-4 animate-spin", getLoaderStyles())}
                />
              </div>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {filteredLabels.length ? (
                ...
                >
                  <span
                    className={cn(
                      ...
                      getDotStyles(label.color, isSelected)
                    )}
                  />
                  {label.name}
                </button>
            ...
            <p className={cn("text-xs", getEmptyTextStyles())}>
              {noMatchText || defaultNoMatchText}
            </p>
          ) : (
            <p className={cn("text-xs", getEmptyTextStyles())}>{emptyText}</p>
          )}
        </div>
      </div>
    </div>
