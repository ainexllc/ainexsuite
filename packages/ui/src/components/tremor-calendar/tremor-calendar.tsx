...
// Based on https://www.tremor.so/docs/inputs/calendar
  ...
  ({ onClick, icon, disabled, ...props }: NavigationButtonProps, forwardedRef) => {
    ...
    return (
      <button
        ...
        disabled={disabled}
        className={cx(
          'flex size-8 shrink-0 select-none items-center justify-center rounded-md border p-1 outline-none transition sm:size-[30px]',
...
const TremorCalendar = ({
  ...
  classNames,
  ...props
}: TremorCalendarProps & { enableYearNavigation?: boolean }) => {
  return (
    <DayPicker
      mode={mode}
      ...
      classNames={{
        ...
        head_cell:
          'w-9 font-medium text-sm sm:text-xs text-center text-zinc-400 dark:text-zinc-500 pb-2',
        row: 'w-full mt-0.5',
        cell: cx(
        ...
        day_selected: cx(
          ...
          'dark:aria-selected:bg-[var(--color-primary)] dark:aria-selected:text-white'
        ),
        day_disabled:
        ...
        Caption: ({ ...captionProps }) => {
          ...
          return (
            <div className="flex items-center justify-between">
              ...
              >
                {format(captionProps.displayMonth, 'LLLL yyy', { locale })}
              </div>
              ...
              {today && (
                <span
                  className={cx(
                    ...
                    {
                      'bg-[var(--color-primary)] dark:bg-[var(--color-primary)]': !selected,
                      'bg-white dark:bg-zinc-950': selected,
                      'bg-zinc-400 dark:bg-zinc-600': selected && range_middle,
                      'bg-zinc-400 text-zinc-400 dark:bg-zinc-400 dark:text-zinc-600': disabled,
                    }
                  )}
                />
              )}
...
TremorCalendar.displayName = 'TremorCalendar';

export { TremorCalendar, type Matcher };
export type { TremorCalendarProps };
