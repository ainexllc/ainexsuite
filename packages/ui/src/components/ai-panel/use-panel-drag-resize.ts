...
/**
 * Panel state representing position and dimensions
 */
...
/**
 * usePanelDragResize - Hook for drag and resize functionality on floating panels
 *
 * Features:
 ...
 * @example
 * ```tsx
 * const { state, dragHandleProps, getResizeHandleProps } = usePanelDragResize({
 *   initialState: { x: 100, y: 100, width: 400, height: 500 },
 *   minWidth: 300,
 *   minHeight: 400,
 ...
 *
 * return (
 *   <div style={{ left: state.x, top: state.y, width: state.width, height: state.height }}>
 *     <div {...dragHandleProps}>Drag Handle</div>
 *     <div {...getResizeHandleProps('corner')} />
 *   </div>
 * );
 * ```
 */
export function usePanelDragResize(options: UsePanelDragResizeOptions) {
  ...
  const constrainToViewport = useCallback(
    (newState: PanelState): PanelState => {
...
export default usePanelDragResize;
